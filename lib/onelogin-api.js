...
function extractPagination(headers) {
  ...
  // Cursor headers are URL-encoded (e.g. a trailing %3D%3D). Decode them so
  // the surfaced value can be passed straight back as a `cursor` arg without
  // being double-encoded by URLSearchParams.
  const decode = (value) => {
    if (!value) return value;
    try { return decodeURIComponent(value); } catch { return value; }
  };
  const beforeCursor = decode(headers.get('before-cursor'));
  const afterCursor = decode(headers.get('after-cursor'));
