 ...
 * Prioritizes environment variables, falls back to servers.json
 * @returns {Object} Server configuration with credentials
 */
export function getActiveServer() {
  // Check if all required credentials are in environment variables
  const envUrl = process.env.ONELOGIN_URL;
  const envClientId = process.env.ONELOGIN_CLIENT_ID;
  const envClientSecret = process.env.ONELOGIN_CLIENT_SECRET;
  
  // If all required env vars present, use them directly
  if (envUrl && envClientId && envClientSecret) {
    const usePreprod = process.env.ONELOGIN_USE_PREPROD === 'true';
    const serverName = process.env.ONELOGIN_SERVER || 'default';
    ...
    return {
      ...
      legacy_key: process.env.ONELOGIN_LEGACY_KEY || '',
      use_preprod: usePreprod
    };
  }
  
  // Fall back to servers.json configuration
  const servers = loadServers();
  const serverNames = Object.keys(servers);
  
  if (serverNames.length === 0) {
    throw new Error(
      'No servers configured. Either set environment variables (ONELOGIN_URL, ONELOGIN_CLIENT_ID, ONELOGIN_CLIENT_SECRET) or run `npx onelogin-mcp-setup` to configure servers.json'
    );
  }

  const config = loadConfig();

  // Check for environment variable override of server name
  const envServer = process.env.ONELOGIN_SERVER;
  const serverName = envServer || (config && config.active_server);

  if (!serverName) {
    throw new Error(
      'No active server configured. Run `npx onelogin-mcp-setup` or set ONELOGIN_SERVER env var'
    );
  }
