...
/**
 * Create Smart Hook environment variables
 * POST /api/2/hooks/{id}/envs
 * @param {OneLoginApi} api
 * @param {Object} args - {hook_id: string, env_vars: object}
 * @returns {Promise<Object>}
 */
export async function createSmartHookEnvVars(api, args) {
  if (!args.hook_id) {
    throw new Error('hook_id is required');
  }
...
export const tools = [
  ...
  {
    name: 'create_smart_hook_env_vars',
    description: 'Create new environment variables for a Smart Hook. Provide key-value pairs as object: {"API_KEY": "secret123", "SERVICE_URL": "https://api.example.com"}. Variables are encrypted and accessible in hook code via process.env.API_KEY. Use for storing secrets, API endpoints, or configuration without hardcoding in function. IMPORTANT: If variables with same names already exist, they will be overwritten. Use this to add new configuration variables to a hook. Returns success status and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        hook_id: {
...
export const handlers = {
  ...
  create_smart_hook_env_vars: createSmartHookEnvVars,
