...
export const tools = [
  ...
  },
  {
    ...
    description: 'Create environment variables for a Smart Hook. Provide key-value pairs as object: {"API_KEY": "secret123", "SERVICE_URL": "https://api.example.com"}. Variables are encrypted and accessible in hook code via process.env.API_KEY. Use for storing secrets, API endpoints, or configuration without hardcoding in function. NOTE: The OneLogin API uses the same endpoint for create and update - if a variable name already exists, its value will be updated. Returns success status and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        hook_id: {
