...
export async function createSmartHook(api, args) {
  ...
  // None of these accept null, so drop null/undefined rather than let them overwrite a
  // default and reintroduce the very 422 the defaults exist to prevent.
...
/**
 * Fields PUT /api/2/hooks/{id} accepts. The endpoint sets additionalProperties: false,
 * so a hook read back from the API cannot be echoed to it verbatim - id, created_at
 * and updated_at have to be dropped.
 */
...
export async function updateSmartHook(api, args) {
  ...
  // Drop null/undefined so an unset field falls back to the hook's current value rather
  // than overwriting it with a value the API rejects.
  const changes = Object.fromEntries(
    Object.entries(args).filter(
      ([field, value]) => field !== 'hook_id' && value !== undefined && value !== null
    )
  );
