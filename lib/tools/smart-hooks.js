...
function normalizeEnvVarNames(envVars) {
  ...
  if (typeof envVars === 'object') {
    // A lone {name, value} pair describes one variable. Reading its keys would ask for
    // variables called "name" and "value" instead of the one the caller meant.
    const keys = Object.keys(envVars);
    const isSinglePair =
      typeof envVars.name === 'string' && keys.every(k => k === 'name' || k === 'value');

    return isSinglePair ? [envVars.name] : keys;
  }
...
export async function listSmartHooks(api, args = {}) {
  ...
  // Filter parameters. The API only filters by type - it ignores anything else,
  // so do not offer a disabled filter that would silently return every hook.
...
/**
 * Fields the hooks API accepts on create and update. Both endpoints set
 * additionalProperties: false, so a hook read back from the API cannot be echoed to it
 * verbatim - id, created_at, updated_at and the read-only status pair have to be dropped.
 */
const WRITABLE_HOOK_FIELDS = [
  'function',
  'type',
  'disabled',
  'runtime',
...
/**
 * Reduce caller args to fields the API will accept.
 *
 * Reading a hook and sending it back with one field changed is the obvious way to use
 * these tools, but the response carries id/created_at/updated_at/status, and both
 * endpoints reject unknown properties - so forwarding args untouched turns that natural
 * call into a 422. Nulls go too: none of these fields accept one, and a caller passing
 * null to "unset" a field would otherwise overwrite a default or a live value.
 * @param {Object} args
 * @returns {Object}
 */
function pickWritableFields(args) {
  return Object.fromEntries(
    Object.entries(args).filter(
      ([field, value]) =>
        WRITABLE_HOOK_FIELDS.includes(field) && value !== undefined && value !== null
    )
  );
}
 ...
 * PUT /api/2/hooks/{id}
 *
 * The API replaces the whole hook and requires function, type, disabled, runtime,
 * timeout, retries, env_vars and packages on every call, so a partial body is
 * rejected with a 422. Read the current hook and merge the caller's changes over it
 * to keep partial updates working.
 * @param {OneLoginApi} api
...
export async function updateSmartHook(api, args) {
  ...
  const changes = pickWritableFields(args);

  const current = await getSmartHook(api, { hook_id: hookId });
  if (!current.success) {
    return current;
  }

  const payload = {};
  for (const field of WRITABLE_HOOK_FIELDS) {
    if (current.data[field] !== undefined && current.data[field] !== null) {
      payload[field] = current.data[field];
    }
  }
  ...
  return await api.put(`/api/2/hooks/${hookId}`, payload);
}
...
export const tools = [
  {
    ...
    description: 'Get a list of all Smart Hooks configured in your OneLogin account. Smart Hooks are serverless functions that execute custom logic in response to OneLogin events. Can filter by hook type - there is NO disabled filter, so to find disabled hooks list them and read each hook\'s disabled field. Returns hook list with IDs, types, status, runtime, created/updated timestamps, and x-request-id. Report the runtime from the response rather than assuming a value - new hooks default to nodejs24.x, but hooks predating the runtime upgrade still report older values. Defaults to 100 per page.',
    inputSchema: {
      type: 'object',
      properties: {
        type: {
          ...
          description: 'Filter by hook type. This is the only filter the API supports; to find disabled hooks, list them and check each hook\'s disabled field.'
        },
        limit: {
          type: 'number',
          description: 'Results per page. Both the default and the MAXIMUM are 100 - a larger value is rejected with a 422.'
        },
        page: {
          type: 'number',
        ...
        cursor: {
          type: 'string',
          description: 'Opaque pagination cursor. Take the value from the After-Cursor or Before-Cursor RESPONSE header of a previous call and pass it back here - there are no separate after_cursor/before_cursor request params.'
        }
      },
  ...
  {
    name: 'get_smart_hook',
    description: 'Get detailed configuration of a specific Smart Hook by ID. Returns complete hook definition including type (determines when it triggers), disabled (whether it runs), status (READ-ONLY deploy state such as ready or update-queued - NOT whether the hook is enabled), runtime environment, function code (base64), packages (npm dependencies), env var names (values are never returned), options, conditions, retries, timeout settings, and x-request-id. Use to review or backup hook logic, and to see which options flags a hook has enabled.',
    inputSchema: {
      type: 'object',
      properties: {
        hook_id: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        options: {
          type: 'object',
          description: 'Opt in to extra data on the hook context. Each flag defaults to false, which means the data it controls is undefined inside the function. pre-authentication ONLY: user-migration defines no options, so passing any flag to one is REJECTED with a 422 (it reports options as {} when read back). These are also editable in the admin console under Settings > Smart Hooks (super users only) as the Options checkboxes on a hook - except failed_login_attempts_enabled, which is API-only.',
          properties: {
            location_enabled: {
              type: 'boolean',
        ...
        packages: {
          type: 'object',
          description: 'npm packages as {"package-name": "version"}. The version must be an EXACT semver, e.g. "4.17.21" - ranges are rejected with a 422, so "^4.17.21", "~1.2", "1.x" and "latest" all fail.'
        },
        env_vars: {
          type: 'array',
          items: { type: 'string' },
        ...
        timeout: {
          type: 'number',
          description: 'Timeout in seconds, default 1. The max DEPENDS ON TYPE: pre-authentication allows only 1 (it blocks the login), user-migration allows 1-10. Passing 5 to a pre-authentication hook is a 422.'
        },
        disabled: {
          type: 'boolean',
        ...
        conditions: {
          type: 'array',
          maxItems: 10,
          description: 'Optional conditional execution rules - the hook only runs when they match. Max 10. Only pre-authentication supports conditions, and its only source is "roles" (user-migration supports none). Values are role IDs.',
          items: {
            type: 'object',
            required: ['source', 'operator', 'value'],
            properties: {
              source: { type: 'string', description: 'Only "roles" is supported' },
              operator: { type: 'string', description: '"~" (includes) or "!~" (excludes)' },
              value: { type: 'string', description: 'Role ID, as a string' }
            }
          }
        },
        context_version: {
          type: 'string',
          description: 'Version of the context object passed to the function. pre-authentication supports 1.0.0 and 1.1.0; defaults to 1.0.0 if omitted, which pins the hook to the older context shape. Set explicitly if the function needs newer context fields.'
        }
      },
  ...
  },
  {
    ...
    description: 'Update an existing Smart Hook configuration. Can modify function code, packages, env var names, retries, timeout, disabled, conditions and options. type CANNOT be changed after creation (attempting it is a 422), and status is read-only - it reports deploy state, so use disabled to enable/disable a hook. Partial updates supported - pass only the fields to change, and this tool reads the current hook and merges them, because the underlying API replaces the whole hook and rejects a partial body. Note this means an update costs a GET plus a PUT, and concurrent updates can overwrite each other. IMPORTANT: Updating function code while hook is enabled may cause errors during deployment - consider disabling first. Use this to turn on options flags (location_enabled, risk_enabled, mfa_device_info_enabled, failed_login_attempts_enabled) on a pre-authentication hook created without them - that is how you fix a hook whose context is missing data. The first three can equally be ticked in the admin console under Settings > Smart Hooks; failed_login_attempts_enabled is API-only. Returns updated hook data and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        hook_id: {
        ...
        function: {
          type: 'string',
          description: 'New base64-encoded JavaScript function code (max 20000 chars)'
        },
        options: {
          type: 'object',
          description: 'Opt in to extra data on the hook context. Each flag defaults to false, which means the data it controls is undefined inside the function. pre-authentication ONLY: user-migration defines no options, so passing any flag to one is REJECTED with a 422 (it reports options as {} when read back). These are also editable in the admin console under Settings > Smart Hooks (super users only) as the Options checkboxes on a hook - except failed_login_attempts_enabled, which is API-only.',
        ...
        packages: {
          type: 'object',
          description: 'New npm packages as {"package-name": "version"}. Version must be an EXACT semver ("4.17.21"); ranges such as "^4.17.21" or "latest" are rejected with a 422.'
        },
        env_vars: {
          type: 'array',
          items: { type: 'string' },
          description: 'Names of existing account env vars this hook may read, e.g. ["API_KEY"]. Names only - values live on the env var itself. Replaces the hook\'s current list.'
        },
        retries: {
          type: 'number',
          description: 'New retry count (0-4)'
        },
        timeout: {
          type: 'number',
          description: 'New timeout in seconds. The max DEPENDS ON TYPE: pre-authentication allows only 1, user-migration allows 1-10.'
        },
        disabled: {
          type: 'boolean',
          description: 'New disabled status. This - not the read-only status field - is what enables or disables a hook.'
        },
        conditions: {
          type: 'array',
          maxItems: 10,
          description: 'Replacement conditional execution rules (max 10). pre-authentication only; its sole source is "roles". Every update rewrites conditions, so pass the full list you want, not just additions.',
          items: {
            type: 'object',
            required: ['source', 'operator', 'value'],
            properties: {
  ...
  },
  {
    ...
    description: 'Permanently delete a Smart Hook. WARNING: This operation cannot be undone. Events will no longer trigger this hook logic. Any automation depending on this hook will stop working. If you don\'t know the hook ID, use list_smart_hooks to find it. Returns 202 Accepted, NOT 204: deletion is asynchronous and only queued, so the hook can still appear in list_smart_hooks and get_smart_hook briefly afterwards. Do not treat its continued presence as a failed delete, and do not retry on that basis. Returns x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        hook_id: {
