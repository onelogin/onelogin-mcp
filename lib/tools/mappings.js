...
export async function createMapping(api, args) {
  ...
  return await api.post('/api/2/mappings', args);
}
...
export const tools = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        position: {
          type: 'number',
          description: 'Execution order position'
        }
      },
  ...
  {
    name: 'update_mapping',
    description: 'Update an existing user mapping configuration. Can modify name, match criteria (all/any), enabled status, position, conditions, and actions. Partial updates supported - only provide fields to change. Use to adjust mapping logic or reorder execution. Returns updated mapping data and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        mapping_id: {
        ...
        name: {
          type: 'string',
          description: 'New mapping name'
        },
        match: {
          type: 'string',
          description: 'New condition match type'
        },
        enabled: {
          type: 'boolean',
        ...
        position: {
          type: 'number',
          description: 'New position in execution order'
        }
      },
      required: ['mapping_id'],
      additionalProperties: true
  ...
  },
  {
    ...
    description: 'Add an action to a mapping that transforms user attributes. Actions execute when conditions match. Common action types: "set_value" (assign static value), "copy_value" (copy from another field), "set_macro" (use OneLogin macro like {firstname}.{lastname}), "set_default_if_null" (fallback value). Value array structure varies by action type. Returns created action with ID and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        mapping_id: {
        ...
        action: {
          type: 'string',
          description: 'Action type (set_value, copy_value, set_macro, etc.)'
        },
        value: {
          type: 'array',
  ...
  },
  {
    ...
    description: 'Update an existing mapping action configuration. Can change action type, target fields, or transformation values. Use to modify how user attributes are transformed without recreating the action. Returns updated action data and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        mapping_id: {
