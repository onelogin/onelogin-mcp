...
export async function listMappings(api, args = {}) {
  ...
  // Filter parameters. These are the only query params the endpoint accepts;
  // anything else (page, limit, after_cursor, before_cursor, has_conditions)
  // is rejected with 422 "Invalid query params".
  if (args.enabled !== undefined) params.enabled = args.enabled;
  if (args.has_condition) params.has_condition = args.has_condition;
  if (args.has_condition_type) params.has_condition_type = args.has_condition_type;
  if (args.has_action) params.has_action = args.has_action;
  if (args.has_action_type) params.has_action_type = args.has_action_type;

  // No pagination: the API returns the full mapping list in one response.
  // Deprecated pagination args (limit, page, after_cursor, before_cursor) are
  // still accepted by the schema for backwards compatibility but ignored.
...
export const tools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_condition: {
          type: 'string',
          description: 'Filter to mappings that have a specific condition, in <name>:<value> form (e.g. "has_role:12345")'
        },
        has_condition_type: {
          type: 'string',
          enum: ['builtin', 'custom', 'none'],
          description: 'Filter by the type of conditions the mapping has: builtin, custom, or none'
        },
        has_action: {
          type: 'string',
          description: 'Filter to mappings that have a specific action, in <name>:<value> form (e.g. "set_status:1")'
        },
        has_action_type: {
          type: 'string',
          enum: ['builtin', 'custom', 'none'],
          description: 'Filter by the type of actions the mapping has: builtin, custom, or none'
        },
        has_conditions: {
          type: 'boolean',
          description: 'DEPRECATED: ignored; the API rejects this param. Use has_condition_type instead'
        },
        limit: {
          type: 'number',
          description: 'DEPRECATED: ignored; the API returns all mappings in one response'
        },
        page: {
          type: 'number',
          description: 'DEPRECATED: ignored; the API returns all mappings in one response'
        ...
        after_cursor: {
          type: 'string',
          description: 'DEPRECATED: ignored; the API does not support cursor pagination for mappings'
        },
        before_cursor: {
          type: 'string',
          description: 'DEPRECATED: ignored; the API does not support cursor pagination for mappings'
