...
export const tools = [
  ...
  },
  {
    ...
    description: 'Get all custom claims defined for an API authorization. Claims are user attributes (email, roles, custom fields) included in access tokens as JWT claims. Returns claim list with IDs, names, user attribute mappings, transformations, and x-request-id. Use to audit what user data is exposed in tokens.',
    inputSchema: {
      type: 'object',
      properties: {
        auth_id: { type: 'number', description: 'The API authorization ID' }
  ...
  },
  {
    ...
    description: 'Remove a claim from an API authorization. Claim will no longer appear in newly issued access tokens. Existing tokens with this claim remain valid until expiration. Use to remove unnecessary user data exposure from tokens. Returns success status and x-request-id.',
    inputSchema: {
      type: 'object',
      properties: {
        auth_id: { type: 'number', description: 'The API authorization ID' },
