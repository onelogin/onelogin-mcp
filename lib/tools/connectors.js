...
export const tools = [
  {
    ...
    description: 'Get a list of all available app connectors in the OneLogin catalog. Connectors are pre-built integration templates for SaaS apps (Salesforce, Slack, AWS, Office 365, etc.). Each connector defines SSO configuration, provisioning rules, and attribute mappings. Filter by name to find specific apps. Paginate with limit/page, or pass a previous response\'s pagination.after_cursor / before_cursor as cursor. Returns connector list with IDs, names, descriptions, logos, auth methods (SAML, OIDC, etc.), a pagination block, and x-request-id. Use to discover available integrations before creating apps.',
    inputSchema: {
      type: 'object',
      properties: {
