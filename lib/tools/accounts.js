...
export async function listAccounts(api, args = {}) {
  ...
  // /api/2/accounts does not paginate: limit/page/after_cursor/before_cursor are
...
export const tools = [
  {
    name: 'list_accounts',
    description: 'List child accounts under your reseller account. Requires reseller privileges. Returns account data including id, name, address, city, state, country, zip, phone, homepage, notes, created_at, and updated_at. Returns the full list in one response (the API does not paginate this endpoint). Returns x-request-id for log tracing.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: ignoredPaginationParam('number', 'accounts'),
        page: ignoredPaginationParam('number', 'accounts'),
        after_cursor: ignoredPaginationParam('string', 'accounts'),
        before_cursor: ignoredPaginationParam('string', 'accounts')
      },
