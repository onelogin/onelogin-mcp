 ...
 * `kind` is here too but is handled separately: settable on create, immutable after.
 */
const SHARED_FIELDS = [
  'name',
  'ip_addr_restriction',
...
export async function listPolicies(api, args = {}) {
  ...
  // Pagination: page numbers, or a cursor taken from a previous response
...
export const tools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        cursor: { type: 'string', description: 'Cursor from a previous response\'s after_cursor/before_cursor to fetch the next or previous page' }
      },
      additionalProperties: false
    }
  },
