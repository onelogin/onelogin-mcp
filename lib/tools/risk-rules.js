...
import { ignoredPaginationParam } from '../pagination.js';
...
export async function listRiskRules(api, args = {}) {
  ...
  // Pagination
  // /api/2/risk/rules does not paginate: limit/page/after_cursor/before_cursor are
  // silently ignored by the API and the full list comes back in one response.
  // They stay in the inputSchema (marked DEPRECATED) but are not forwarded.
...
export const tools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        limit: ignoredPaginationParam('number', 'risk rules'),
        page: ignoredPaginationParam('number', 'risk rules'),
        after_cursor: ignoredPaginationParam('string', 'risk rules'),
        before_cursor: ignoredPaginationParam('string', 'risk rules')
      },
