 ...
 *
 * Body-shape note: the /api/1 role endpoints take an object envelope
 * (`{role_id_array: [...]}`), while the /api/2 sub-resource endpoints
 * (roles/{id}/apps, roles/{id}/admins) take a bare JSON array.
 */
...
export async function listRoles(api, args = {}) {
  ...
  // A roles cursor encodes limit/page/sort and the endpoint rejects a cursor
  // combined with any of them, so sort is only sent when there is no cursor.
  const cursor = applyV2Pagination(params, args);
  if (args.sort && !cursor) params.sort = args.sort;
...
export async function setRoleApps(api, args) {
  ...
  // The v2 endpoint expects a bare JSON array as the body. Wrapping it in an
  // object (v1-style `{app_id_array: [...]}`) is rejected with
  // 400 "Expected array in request".
  return await api.put(`/api/2/roles/${args.role_id}/apps`, args.app_ids);
}
...
export async function addRoleAdmins(api, args) {
  ...
  // The v2 endpoint expects a bare JSON array as the body. Wrapping it in an
  // object (v1-style `{admin_id_array: [...]}`) is rejected with
  // 400 "Expected array in request".
  return await api.post(`/api/2/roles/${args.role_id}/admins`, args.admin_ids);
}
...
export const tools = [
  {
    name: 'list_roles',
    description: 'Get a list of roles with pagination and sorting support (max 650 per page). Can filter by name, app_id, or app_name. Use fields parameter to include related data (apps, users, admins) in response. Paginate with limit/page, or pass a previous response\'s pagination.after_cursor / before_cursor as cursor (sent alone; the cursor already encodes limit and sort). Returns role data, a pagination block, and x-request-id for log tracing.',
    inputSchema: {
      type: 'object',
      properties: {
