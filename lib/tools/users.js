...
/**
 * List custom attribute field definitions (schema)
 * GET /api/2/users/custom_attributes
 * Returns the list of custom field shortnames available for users
 * @param {OneLoginApi} api
 * @returns {Promise<Object>}
 */
export async function listUserCustomAttributes(api) {
  return await api.get(`/api/2/users/custom_attributes`);
}

/**
 * Get a specific custom attribute field definition
 * GET /api/2/users/custom_attributes/{id}
 * @param {OneLoginApi} api
 * @param {Object} args - {custom_attribute_id: number}
 * @returns {Promise<Object>}
 */
export async function getUserCustomAttribute(api, args) {
  if (!args.custom_attribute_id) {
    throw new Error("custom_attribute_id is required");
  }

  return await api.get(`/api/2/users/custom_attributes/${args.custom_attribute_id}`);
}

/**
 * Create a new custom attribute field definition
 * POST /api/2/users/custom_attributes
 * @param {OneLoginApi} api
 * @param {Object} args - {name: string, shortname: string, ...other fields}
 * @returns {Promise<Object>}
 */
export async function createUserCustomAttribute(api, args) {
  ...
  return await api.post("/api/2/users/custom_attributes", args);
}

/**
 * Update a custom attribute field definition
 * PUT /api/2/users/custom_attributes/{id}
 * @param {OneLoginApi} api
 * @param {Object} args - {custom_attribute_id: number, ...fields to update}
 * @returns {Promise<Object>}
 */
export async function updateUserCustomAttribute(api, args) {
  ...
  return await api.put(`/api/2/users/custom_attributes/${customAttributeId}`, updateData);
}

/**
 * Delete a custom attribute field definition
 * DELETE /api/2/users/custom_attributes/{id}
 * @param {OneLoginApi} api
 * @param {Object} args - {custom_attribute_id: number}
 * @returns {Promise<Object>}
 */
export async function deleteUserCustomAttribute(api, args) {
  ...
  return await api.delete(`/api/2/users/custom_attributes/${args.custom_attribute_id}`);
}
...
export const tools = [
  ...
  },
  {
    name: "update_user",
    description:
      "Update user attributes including passwords, group membership, trusted IDP, and custom attributes. CUSTOM ATTRIBUTES: Set values using the custom_attributes object, e.g., {\"custom_attributes\": {\"employee_id\": \"12345\"}}. This is the correct way to update custom attribute values for a user. OTHER OPTIONS: Mappings run async by default (use mappings=sync for immediate response). Passwords validate against User Policy by default (use validate_policy=false to skip). Returns updated user data and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {
        user_id: {
  ...
  },
  {
    ...
    description:
      "Get a list of all custom attribute field definitions (schema) available in your OneLogin account. Returns the shortnames of custom fields that can be used when setting custom_attributes in create_user or update_user. IMPORTANT: This returns the field SCHEMA (which fields exist), NOT user-specific values. To set values for a user, use update_user with the custom_attributes object. Example: If this returns [\"employee_id\", \"department\"], you can then set these via update_user: {\"custom_attributes\": {\"employee_id\": \"12345\"}}. Returns custom attribute shortnames and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      "Get details of a specific custom attribute field definition by ID. Returns the custom attribute schema including name, shortname, user-editable flag, and x-request-id. Use list_user_custom_attributes to get a list of all custom attribute IDs.",
    inputSchema: {
      type: "object",
      properties: {
        custom_attribute_id: {
  ...
  },
  {
    ...
    description:
      "Create a new custom attribute field definition in OneLogin. This creates the field schema (definition), not a value for a specific user. After creating the field, you can set values for individual users using update_user with custom_attributes. Returns the created custom attribute definition and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {
        name: {
        ...
        shortname: {
          type: "string",
          description:
            "The shortname used to reference this attribute in API calls (e.g., 'employee_id')",
        },
        user_editable: {
          type: "boolean",
          description:
            "Whether users can edit this field themselves (default: false)",
        },
      },
  ...
  },
  {
    ...
    description:
      "Update an existing custom attribute field definition. This updates the field schema (definition), not values for specific users. To update values for a user, use update_user with custom_attributes instead. Returns the updated custom attribute definition and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {
        custom_attribute_id: {
          type: "number",
          description: "The custom attribute ID to update",
        },
        name: {
          type: "string",
          description: "The display name of the custom attribute",
        ...
        shortname: {
          type: "string",
          description: "The shortname used to reference this attribute",
        },
        user_editable: {
          type: "boolean",
          description: "Whether users can edit this field themselves",
        },
      },
      required: ["custom_attribute_id"],
      additionalProperties: true,
  ...
  },
  {
    ...
    description:
      "Delete a custom attribute field definition from OneLogin. WARNING: This operation cannot be undone. Deleting a custom attribute field will remove the field definition and all values stored in this field for all users. Returns 204 No Content on success and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {
        custom_attribute_id: {
          type: "number",
          description: "The custom attribute ID to delete",
        },
      },
      required: ["custom_attribute_id"],
      additionalProperties: false,
