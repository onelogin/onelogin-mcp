...
export const tools = [
  ...
  },
  {
    name: "update_user",
    description:
      "Update user attributes including passwords, group membership, trusted IDP, and custom attributes. To set custom attribute values, use the custom_attributes object (e.g., {\"custom_attributes\": {\"employee_id\": \"12345\"}}). Mappings run async by default (use mappings=sync query parameter to get mapped values in response). Passwords validate against User Policy by default (use validate_policy=false to skip). Returns updated user data and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {
        user_id: {
  ...
  },
  {
    ...
    description:
      "Get a list of all custom attribute field definitions (schema) available in your OneLogin account. Returns the shortnames of custom fields that can be used when setting custom_attributes in create_user or update_user. IMPORTANT: This returns the field schema (which fields exist), not user-specific values. To set values for a user, use update_user with the custom_attributes object. Returns custom attribute shortnames and x-request-id.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
